...
async def get_hashtag_videos(hashtag, num_data=100):
    ...
    response = Response()
    video_data_list = []
    data_dir = "data/json/hashtag"
    file_path = f"{data_dir}/{hashtag}_data.json"
    ...
    if os.path.exists(file_path):
        ...
        response.message = "Hashtag videos data retrieved from storage."
        response.data["videos"] = video_data_list
        response.data["total"] = len(video_data_list)

        print(f"{Fore.GREEN}Hashtag videos data retrieved from storage.")
    ...
    async with TikTokApi() as api:
        ...
        try:
            ...
            # Set response data
            response.success = True
            response.message = "Hashtag videos data has been retrieved and saved successfully."
            response.data["videos"] = video_data_list
            response.data["total"] = len(video_data_list)

            print(f"{Fore.GREEN}Hashtag videos data has been retrieved and saved successfully.")
            return response.to_dict()
